...
<?php
  ...
  else if (isset($_POST['login']))
  {
    ...
    {
      echo "<script>alert('密碼或帳號不可為空'); location.href = './gameboard_outdoor.php';</script>";
    }
    else
    {
      ...
      {
        echo "<script>alert('登錄失敗,跳轉首頁'); location.href = './gameboard_outdoor.php';</script>";
      }
    }
  }
?>
...
max-height: 40px;"/></a>';
        		...
        		<div class="nav-li" style="background:#232323;">
            		<li class="dropdown" style="line-height: 40px; display: block;">
            			...
            			<ul class="dropdown-menu" role="menu" style="background-color: #000;">
              				...
              				<li class="active"><a href="gameboard_outdoor.php">校外各盃賽</a></li>
              				<li class="divider" style="background-color: #333; border-bottom: 0.5px solid #333;"></li>
        		...
        		<div class="nav-li">
          			<li class="dropdown" style="line-height: 40px; display: block; background-color:none;">
	...
	<div id="container" style="background-color:#efefef; box-shadow:1px 1px 80px 20px rgba(20%,20%,20%,0.4) inset;"> 
      ...
      <ul class="breadcrumb" style="background-color: transparent; no-repeat; padding: 5px 15px;">
        ...
        <li class="active"><a style="color:#fff;">GameBoard(outdoor)</a></li>
      </ul>
      <?php
        try
        {
          $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $row = $db->query("SELECT * FROM `game` WHERE gClass = '校外'");
          ...
          while ($r = $row->fetch(PDO::FETCH_ASSOC))
          {
            echo '<tr class="clickable-row" data-href="scoreboard_outdoor.php?gId='.$r['gId'].'">';
            echo "<td>".$r['gTime']."</td>";
            echo "<td>".$r['gTitle']."</td>";
